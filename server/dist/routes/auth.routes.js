...
router.post('/register', async (req, res) => {
    try {
        const { email, password, name, age, gender, bio, location, interested_in, terms_accepted, 
        // Founder Journey fields (Step 2)
        founder_status, industry, funding_status, company_name, company_stage, role, founding_story, biggest_challenge, 
        // Lifestyle & Compatibility fields (Step 3)
        work_life_balance, ideal_date_night, view_on_exit, date_people_who_talk_work, how_destress, coffee_order, hustle_quote } = req.body;
        // Validation
    ...
    `).run(email, passwordHash, name, age, gender, bio || null, location || null, JSON.stringify([]), JSON.stringify(interested_in || ['male', 'female', 'non-binary']), founder_status || null, industry || null, funding_status || null, company_name || null, company_stage || null, role || null, founding_story || null, biggest_challenge || null, work_life_balance || null, ideal_date_night || null, view_on_exit || null, date_people_who_talk_work || null, how_destress || null, coffee_order || null, hustle_quote || null, 1 // terms_accepted = true
        );
...
// Delete current user
router.delete('/me', auth_1.authenticateToken, (req, res) => {
    try {
        const db = (0, db_1.getDatabase)();
