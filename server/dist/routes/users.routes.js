    ...
    `).all(userId, ...interestedIn, ageMin, ageMax, userId);
        ...
        const parsedUsers = users
            ...
            if (currentUser.latitude && currentUser.longitude && user.latitude && user.longitude) {
                parsedUser.distance = Math.round(calculateDistance(currentUser.latitude, currentUser.longitude, user.latitude, user.longitude));
            }
...
router.get('/profile/:id', auth_1.authenticateToken, (req, res) => {
    try {
        const db = (0, db_1.getDatabase)();
        const { id } = req.params;
...
router.put('/profile', auth_1.authenticateToken, (req, res) => {
    try {
        const db = (0, db_1.getDatabase)();
        const userId = req.userId;
        const { name, age, bio, location, latitude, longitude, photos, interested_in, age_min, age_max, distance_max, founder_status, industry, funding_status, company_name, company_stage, role, founding_story, biggest_challenge, work_life_balance, ideal_date_night, view_on_exit, date_people_who_talk_work, how_destress, coffee_order, hustle_quote } = req.body;
        const updates = [];
        const values = [];
        if (name !== undefined) {
            updates.push('name = ?');
...
router.get('/stats', auth_1.authenticateToken, (req, res) => {
    try {
        const db = (0, db_1.getDatabase)();
        const userId = req.userId;
        const matches = db.prepare(`SELECT COUNT(*) as count FROM matches WHERE user1_id = ? OR user2_id = ?`).get(userId, userId);
        const profileViews = db.prepare(`SELECT COUNT(*) as count FROM swipes WHERE swiped_id = ?`).get(userId);
        const likes = db.prepare(`SELECT COUNT(*) as count FROM swipes WHERE swiper_id = ? AND direction = 'right'`).get(userId);
        res.json({
